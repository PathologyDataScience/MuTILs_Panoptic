...
CONTENT_3="
    ...
    @staticmethod
    def _remove_dark_pixels(slide, threshold=60):
        \"\"\"Remove dark pixels from the thumbnail.\"\"\"
        grayscale = slide.thumbnail.convert("L")

        def _threshold_filter(pixel):
            return 255 if pixel < threshold else pixel

        mask = grayscale.point(_threshold_filter)

        return Image.composite(Image.new("RGB", slide.thumbnail.size, "white"), slide.thumbnail, mask)
"
# --------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------
CONTENT_6="        thumb = self._remove_dark_pixels(slide)"
# --------------------------------------------------------------------------------------------------
...
if ! grep -Fxq "from PIL import Image" "$TARGET_FILE_2"; then
    ...
    sed -i "95,96d" "$TARGET_FILE_2"
    sed -i "94r /dev/stdin" "$TARGET_FILE_2" <<< "$CONTENT_4"
    # sed -i "98r /dev/stdin" "$TARGET_FILE_2" <<< "$CONTENT_5"
    sed -i "98r /dev/stdin" "$TARGET_FILE_2" <<< "$CONTENT_6"
else
